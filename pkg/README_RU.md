...
| **[rest](rest/)** | Обертка HTTP-клиента с автоматическими повторами и экспоненциальной задержкой. |
...
1. **Изолированное тестирование (Mockability):** Структуры зависят от лаконичных интерфейсов (таких как `transport.Doer` или `storage.Store`), а не от конкретных реализаций, что упрощает написание модульных тестов.
2. **Конкурентность на базе каналов:** Передача событий внутри системы осуществляется через пакет `bus` во избежание взаимных блокировок. Совместно используемое состояние защищено при помощи `sync/atomic` и RWMutex.
